postgres:
	docker run --name postgres12 --network bank-network -p 5433:5432 -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=secret -d postgres:12-alpine
createdb:
...
mock:
	mockgen --package mockdb  --destination db/mock/store.go  github.com/PhilipFelipe/simple-bank/db/sqlc Store
